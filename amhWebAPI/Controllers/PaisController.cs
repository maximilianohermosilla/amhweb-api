...
namespace MayiBeerCollection.Controllers
{
    ...
    public class PaisController : ControllerBase
    {
        ...
        {
            List<Pais> lst = (from tbl in _contexto.Pais where tbl.Id > 0 select new Pais() { Id = tbl.Id, Nombre = tbl.Nombre, IdArchivo = tbl.IdArchivo }).OrderBy(e => e.Nombre).ToList();
        ...
        public ActionResult<PaisDTO> Pais(int PaisId)
        {
            ...
            var cl = (from tbl in _contexto.Pais where tbl.Id == PaisId select new Pais() { Id = tbl.Id, Nombre = tbl.Nombre, IdArchivo = tbl.IdArchivo }).FirstOrDefault();
            if (cl == null)
            {
